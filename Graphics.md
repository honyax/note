...
1. 前提となる知識とDirectX 12の概略
   ...
   - ディスクリプタ<br/>
     GPUに送ったデータをシェーダーで使用するための仕様書。シェーダーリソースビューやサンプラーステートなどが代表的。<br/>
     DirectX11にあったビューの上位概念。<br/>
   - ディスクリプタヒープ<br/>
     ディスクリプタの置き場。テクスチャや定数などのアドレスを1箇所にまとめておくためのヒープ領域。<br/>
   - ディスクリプタテーブル<br/>
     ディスクリプタヒープとシェーダーのレジスタを関連付ける、連想配列やテーブルのようなイメージ。<br/>
     ルートシグネチャ内で設定するもの。<br/>
     ディスクリプタヒープがシェーダーから使用されるときに、「どのレジスタに」「どのデータが割り当てられているのか」を記述する。<br/>
     ディスクリプタレンジがこの下につく。<br/>
   - ルートシグネチャ<br/>
     ディスクリプタテーブルをまとめるもの。サンプラーやディスクリプタ単品、データの塊もまとめたりする。<br/>
   - パイプラインステート<br/>
     表示までの流れに必要な、「頂点レイアウト」「頂点シェーダー」「ピクセルシェーダー」「ラスタライザーの設定」などのパラメータを1つのオブジェクトにまとめたもの。<br/>
     パイプラインステートを作成する際にはルートシグネチャオブジェクトが必要となる。<br/>
   - コマンドリストとコマンドキュー<br/>
     GPUにやってほしい仕事をまとめたもの。<br/>
     DirectX11までは忘我系の命令などはデバイスコンテキスト（DirectX10まではデバイス）に対してバラバラのタイミングで命令していた。<br/>
     DirectX12では、コマンドリストというリストに命令オブジェクトをためて、コマンドキューに命令して一気に実行させる。<br/>
     ただし、GPU側にコマンドリストを投げるとすぐにCPU側に処理が復帰するため、次のフェンスを使って描画命令の完了を待つ必要がある。<br/>
   - フェンス<br/>
     GPU側の処理が終わったかどうかを監視する仕組み。<br/>
   - バリア<br/>
     コマンドを投げたときに、リソースが何の用途に使用されるのかを設定するためのもの。<br/>
     CPUからコピーされたリソース（GPU上のメモリ領域）が、レンダーターゲットやテクスチャ、深度参照元、画面フリップ元などになる。<br/>
...
8. マテリアル（材質）
   ...
   - 定数バッファービュー（CBV）とシェーダーリソースビュー（SRV）を組み合わせる
   - ランバートの余弦則で輝度を計算する
   - トゥーンシェーディングでは、輝度の変化に手を加えている。
     - 輝度の値を、なめらかなグラデーションでなくパキッと色が変わるカラールックアップテーブルを使う。
9. リファクタリング
   - ここまでのコードをキレイにリファクタリングしている。
10. スキニングとアニメーション
    - スキニングにより、関節が曲がっても違和感が無い形で見せる
    - ボーンの構造と動かし方、階層構造でのデータの扱い方<br/>
      - 関節の位置、回転の算出例<br/>
        肩：肩回転<br/>
        ひじ：肩回転 * ひじ回転<br/>
        手首：肩回転 * ひじ回転 * 手首回転<br/>
        回転は、「座標を原点に戻す=>回転する=>元の座標に戻す」の一連の流れ
    - アニメーション（数学的な説明がある）
      - MMDのアニメーションデータ（VMDデータ）の説明
      - モーションは、キーフレームを打った場所にデータを設定する。キーフレーム間のモーションはベジェ補間により補間される
      - クォータニオンと回転行列
        - クォータニオンは、複素平面（極座標）を3D空間に拡張したもの。<br/>
          Q = xi + yj + zk + w（x, y, z, wは実数、i, j, kは四元数の単位） <br/>
          また、以下の式を満たす<br/>
